...
| `SUPABASE_URL` | `src/lib/admin-content/storage/index.ts` | Server-side Supabase project URL (preferred for admin upload storage; accepted as alias alongside `NEXT_PUBLIC_SUPABASE_URL`) |
...
| `EDUNANCIAL_UPLOAD_STORAGE_KEY` | `src/lib/admin-content/storage/index.ts` | Supabase storage bucket for admin uploads and exports (`EDUNANCIAL_UPLOAD_STORAGE_BUCKET` is also accepted; defaults to `admin-content` when neither is set) |
