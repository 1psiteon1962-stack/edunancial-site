...
## Administrator and Executive authentication

| Variable | Used in | Purpose |
|---|---|---|
| `EDUNANCIAL_ADMIN_EMAIL` | `src/lib/admin-content/auth.ts` | Administrator login identity |
| `EDUNANCIAL_ADMIN_PASSWORD_HASH` | `src/lib/admin-content/auth.ts` | Server-side scrypt password hash for the administrator account |
| `EDUNANCIAL_OWNER_EMAIL` | `src/lib/admin-content/auth.ts` | Executive / owner login identity |
| `EDUNANCIAL_OWNER_PASSWORD_HASH` | `src/lib/admin-content/auth.ts` | Server-side scrypt password hash for the executive / owner account |
| `EDUNANCIAL_ADMIN_SESSION_SECRET` | `src/lib/admin-content/auth.ts` | Shared HMAC signing key for administrator and executive sessions |
