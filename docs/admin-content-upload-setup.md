...
- `EDUNANCIAL_UPLOAD_STORAGE_KEY` (or `EDUNANCIAL_UPLOAD_STORAGE_BUCKET`)
...
Create a Supabase storage bucket whose name matches `EDUNANCIAL_UPLOAD_STORAGE_KEY` (or `EDUNANCIAL_UPLOAD_STORAGE_BUCKET` if you use the alias).
The portal stores:
- batch JSON metadata
- audit history
- original uploads
