...
function localeFromFilename(canonicalPath, localizedPath) {
  ...
  const match = name.match(new RegExp(`^${base.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&')}\\.([A-Za-z0-9-]+)\\.md$`, 'u'));
  return match?.[1] || null;
}
...
function localInventory() {
  ...
  for (const lesson of lessons) {
    ...
    if (!existsSync(canonical)) throw new Error(`Registry lesson ${lesson.id} is missing file ${lesson.path}`);
...
function addTranslationJsonLocalesFromWorktree(inventory) {
  for (const relDir of COURSE_DIRS) {
    for (const file of walk(join(ROOT, relDir))) {
      ...
      for (const locale of Object.keys(parsed.translations || {})) inventory.get(id).locales.add(locale);
    }
  }
}
...
function refInventory(ref) {
  ...
  const result = new Map(lessons.map((lesson) => [lesson.id, { ...lesson, locales: new Set(['en']) }]));
  let files = [];
  try {
    files = execFileSync('git', ['ls-tree', '-r', '--name-only', ref], { encoding: 'utf8' }).split(/\r?\n/u).filter(Boolean);
  } catch {
    return result;
  }
  ...
  for (const file of files.filter((f) => COURSE_DIRS.some((d) => f.startsWith(`${d}/`)) && f.endsWith('-translations.json'))) {
    ...
    try {
      ...
      for (const locale of Object.keys(parsed.translations || {})) result.get(id).locales.add(locale);
    } catch { /* malformed JSON is reported for the head worktree by normal validation */ }
  }
  return result;
}
...
function approvedRemovals() {
  ...
  return new Set((parsed.removals || []).map((r) => String(r.id || '').toUpperCase()).filter(Boolean));
}
...
function validateHead(head) {
  ...
  for (const [id, lesson] of head) {
    ...
    for (const locale of lesson.locales) {
      if (!LOCALE_RE.test(locale)) errors.push(`Malformed locale ${locale} on ${id}`);
    }
  }
  return errors;
}
...
for (const [id, previous] of base) {
  ...
  if (!current) {
    if (!removals.has(id)) errors.push(`Missing production lesson: ${id} (${previous.path})`);
    continue;
  }
  for (const locale of previous.locales) {
    if (!current.locales.has(locale) && !removals.has(id)) errors.push(`Missing production translation: ${id} locale=${locale}`);
  }
}
...
const report = {
  ...
  baseTranslations: [...base.values()].reduce((n, x) => n + x.locales.size, 0),
  proposedTranslations: [...head.values()].reduce((n, x) => n + x.locales.size, 0),
  errors,
};
