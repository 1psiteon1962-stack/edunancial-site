...
const supported=localeRegistry.locales
  .filter(entry=>entry.status==="active"&&!["en-US","en-GB"].includes(entry.locale))
...
async function translate(a,raw,locale){
  ...
  const res=await fetch("https://api.openai.com/v1/chat/completions",{
    ...
    body:JSON.stringify({
      model,
      messages:[
        {role:"system",content:`You are a precise English-to-${locale} curriculum translator. Output JSON only.`},
  ...
  if(!res.ok){
    ...
    const error=new Error(`${a.id}: model API ${res.status} ${body}`);
    error.nonRetryable=res.status===401||res.status===403||res.status===429;
    throw error;
  }
  const j=await res.json();
  const text=j.choices?.[0]?.message?.content;
  if(!text) throw new Error(`${a.id}: empty model response`);
  const t=JSON.parse(text);
  if(!t.title||!t.body||!good(t.body,locale)) throw new Error(`${a.id}: translation quality gate failed`);
...
for(const locale of locales){
  ...
  const workers=Array.from({length:Math.min(6,selected.length)},async()=>{
    while(true){
      ...
      const a=selected[i];
      const dest=outPath(a,locale);
      if(!force&&fs.existsSync(dest)&&good(fs.readFileSync(dest,"utf8"),locale)){
        console.log(`[${locale} ${i+1}/${selected.length}] keep ${a.id}`);
        continue;
      }
      ...
      for(let attempt=1;attempt<=4;attempt++){
        try{
          const txt=await translate(a,raw,locale);
