...
const requiredFiles = [
  ...
  "src/components/admin-content/ResilientUploadClient.tsx",
  "src/components/admin-content/RecoveryClient.tsx",
  "src/app/api/admin/content/upload/recover/route.ts",
  "src/lib/admin-content/storage/index.ts",
  "src/lib/admin-content/upload-operations.ts",
...
for (const phrase of ["begin with financial literacy, build financial competency, and work toward financial intelligence", "first three lessons of Level 1 free"]) if (!i18n.toLowerCase().includes(phrase.toLowerCase())) failures.push(`Canonical public positioning missing: ${phrase}`);

const checkout = await text("src/app/api/square/payment-link/route.ts");
for (const marker of ["EDUNANCIAL_RUNTIME_TAX_ENFORCEMENT_ENABLED", "charge-adapter-disabled", "resolveCheckoutTax", "assertCountryOperationAllowed"]) if (!checkout.includes(marker)) failures.push(`Checkout launch guard missing: ${marker}`);

const accessControl = await text("src/lib/curriculum/access.ts");
for (const marker of ["lessonNumber >= 1 && lessonNumber <= 3", "if (level === 1 || level === 2) return \"basic\"", "if (level === 3 || level === 4) return \"pro\"", "if (isAdmin) return true"]) if (!accessControl.includes(marker)) failures.push(`Learner/owner access contract missing: ${marker}`);
...
// Curriculum upload readiness is GitHub-only. Do not reintroduce Supabase tables,
// migrations, or recovery endpoints as uploader launch requirements.
const uploadStorage = await text("src/lib/admin-content/storage/index.ts");
for (const marker of ["GithubAdminContentStorage", "admin-content-storage", ".edunancial-admin-content"]) if (!uploadStorage.includes(marker)) failures.push(`GitHub uploader storage guard missing: ${marker}`);
const uploadOperations = await text("src/lib/admin-content/upload-operations.ts");
if (uploadOperations.toLowerCase().includes("supabase")) failures.push("Curriculum upload operation audit must not depend on Supabase.");
const finalizeRoute = await text("src/app/api/admin/content/upload/finalize/route.ts");
if (finalizeRoute.toLowerCase().includes("supabase")) failures.push("Curriculum upload finalization must not depend on Supabase.");
const recoveryRoute = await text("src/app/api/admin/content/upload/recover/route.ts");
for (const marker of ["persistent-storage", "getAdminContentStorage", "recoveredWithoutReupload"]) if (!recoveryRoute.includes(marker)) failures.push(`GitHub upload recovery guard missing: ${marker}`);

const translationReadiness = await text("src/lib/curriculum/translation-readiness.ts");
for (const marker of ["north", "america", "level"]) if (!translationReadiness.toLowerCase().includes(marker)) failures.push(`North America curriculum translation readiness marker missing: ${marker}`);

const videoReadiness = await text("src/app/api/admin/video/readiness/route.ts");
for (const marker of ["video_projects", "video_assets", "video_jobs", "video_scenes", "raw-videos", "processed-videos", "WORKER_BASE_URL", "WORKER_SHARED_SECRET", "/health"]) if (!videoReadiness.includes(marker)) failures.push(`Video production readiness guard missing: ${marker}`);

const storageRepair = await text("src/app/api/admin/video/readiness/storage/route.ts");
for (const marker of ["raw-videos", "processed-videos", "createBucket", "public: false"]) if (!storageRepair.includes(marker)) failures.push(`Video storage self-repair guard missing: ${marker}`);

const worker = await text("video-worker/server.mjs");
for (const marker of ["/health", "WORKER_SHARED_SECRET", "ffmpeg", "processed-videos", "video_worker_requests"]) if (!worker.includes(marker)) failures.push(`Video worker runtime marker missing: ${marker}`);

const workerWorkflow = await text(".github/workflows/video-worker-validation.yml");
for (const marker of ["node --check video-worker/server.mjs", "docker build --file video-worker/Dockerfile video-worker"]) if (!workerWorkflow.includes(marker)) failures.push(`Video worker CI validation missing: ${marker}`);

const renderBlueprint = await text("render.yaml");
for (const marker of ["video-worker/Dockerfile", "healthCheckPath: /health", "WORKER_SHARED_SECRET", "SUPABASE_SERVICE_ROLE_KEY"]) if (!renderBlueprint.includes(marker)) failures.push(`Video worker deployment blueprint missing: ${marker}`);
...
console.log("PASS: North America launch-critical checkout, GitHub-only curriculum upload recovery, curriculum readiness, owner workflow, learner gating, KPI surfaces, and video production controls are present.");
