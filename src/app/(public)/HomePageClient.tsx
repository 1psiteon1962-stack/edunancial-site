...
export default function HomePageClient(){
 ...
 return <main className="bg-white text-[#061a36]">
  ...
  <section className="bg-gradient-to-b from-[#eaf7ff] to-white"><div className="mx-auto max-w-[1440px] px-5 py-4 sm:px-8 lg:px-16"><div className="text-center"><h2 className="text-[27px] font-black tracking-[-.02em]">{ui.goalsTitle}</h2><p className="text-[13px] text-slate-600">{ui.goalsBody}</p></div><div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-6">{goals.map((goal,i)=>{const [title,body]=ui.goals[goal];return <Link href={`/assessment?goal=${goal}`} key={goal} className="group overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm"><div className="h-[74px] overflow-hidden bg-slate-100" aria-hidden="true"><div className="h-full w-full bg-no-repeat transition-transform duration-200 group-hover:scale-[1.02]" style={{backgroundImage:"url('/images/home/goals/approved-goals.jpg')",backgroundSize:"600% 100%",backgroundPosition:`${i*20}% center`}}/></div><div className="p-3"><div className="flex justify-between gap-2"><h3 className="text-[12px] font-black leading-4">{title}</h3><span className="font-black">›</span></div><p className="mt-1 text-[10px] leading-[13px] text-slate-600">{body}</p></div></Link>})}</div><div className="mt-3 flex items-center justify-between gap-4 rounded-md bg-[#d7f0ff] px-6 py-3"><div><h3 className="text-[15px] font-black">{copy.assessmentLabel}</h3><p className="text-[11px] text-slate-600">{copy.assessmentBody}</p></div><Link href="/assessment" className="shrink-0 rounded-md bg-[#0797e8] px-8 py-3 text-[12px] font-bold text-white shadow">{copy.assessmentCta}</Link></div></div></section>
 </main>;
}
