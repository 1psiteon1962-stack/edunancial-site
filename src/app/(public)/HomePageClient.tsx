...
import { HOME_GOAL_IMAGE_PATHS, HOME_GOALS } from "@/lib/content-loader/home-goal-images";
...
export default function HomePageClient(){
 ...
 return <main className="bg-white text-[#061a36]">
  <section className="relative isolate min-h-[355px] overflow-hidden bg-[#07345b] text-white lg:min-h-[385px]"><img src="/approved-home-hero.svg" alt="" className="absolute inset-0 h-full w-full object-cover object-center brightness-110 saturate-125"/><div className="absolute inset-0 bg-gradient-to-r from-[#06233f]/90 via-[#06233f]/35 to-transparent"/><div className="relative mx-auto flex min-h-[355px] max-w-[1440px] items-center px-5 py-7 sm:px-8 lg:min-h-[385px] lg:px-16"><div className="max-w-[570px]"><h1 className="text-[38px] font-black leading-[.96] tracking-[-.035em] sm:text-[50px] lg:text-[54px]">{copy.heroTitle}</h1><p className="mt-3 max-w-[510px] text-[15px] font-medium leading-6 sm:text-base">{copy.heroBody}</p><div className="mt-4 flex flex-wrap gap-3"><Link href="/curriculum" className="rounded-md bg-[#ffd52a] px-8 py-3 text-[14px] font-black text-[#061a36] shadow">{copy.startFree}</Link><Link href="/assessment" className="rounded-md border border-white bg-[#061a36]/55 px-8 py-3 text-[14px] font-bold text-white">{copy.takeAssessment}</Link></div></div></div></section>
  ...
  <section className="mx-auto max-w-[1440px] px-5 py-5 sm:px-8 lg:px-16"><div className="flex items-end justify-between gap-4"><div><h2 className="text-[25px] font-black tracking-[-.02em]">{copy.colorTitle}</h2><p className="text-[13px] text-slate-600">{copy.colorLabel}</p></div><Link href="/curriculum" className="shrink-0 text-[13px] font-bold text-[#0797e8]">{copy.explorePathway} →</Link></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">{copy.tracks.map((track,i)=><Link href={track.href} key={track.code} className="group relative min-h-[154px] overflow-hidden rounded-md border-[4px] bg-[#071426] shadow-md" style={{borderColor:trackColors[i]}}><img src={trackPhotos[i]} alt="" className="absolute inset-0 h-full w-full object-cover brightness-110 saturate-125 transition group-hover:scale-105"/><div className="absolute inset-x-0 bottom-0 min-h-[78px] bg-gradient-to-t from-black/95 via-black/75 to-transparent p-3 pt-7"><h3 className="text-[14px] font-black leading-4 text-white">{track.title}</h3><p className="mt-1 line-clamp-2 text-[10px] leading-[13px] text-white/90">{track.body}</p></div></Link>)}</div></section>
  <section className="bg-gradient-to-b from-[#eaf7ff] to-white"><div className="mx-auto max-w-[1440px] px-5 py-4 sm:px-8 lg:px-16"><div className="text-center"><h2 className="text-[27px] font-black tracking-[-.02em]">{ui.goalsTitle}</h2><p className="text-[13px] text-slate-600">{ui.goalsBody}</p></div><div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-6">{HOME_GOALS.map((goal)=>{const [title,body]=ui.goals[goal];return <Link href={`/assessment?goal=${goal}`} key={goal} className="group overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm"><div className="h-[74px] overflow-hidden bg-slate-100"><img src={`${HOME_GOAL_IMAGE_PATHS[goal]}?v=735`} alt={title} loading="eager" decoding="async" className="block h-full w-full object-cover transition-transform duration-200 group-hover:scale-[1.02]"/></div><div className="p-3"><div className="flex justify-between gap-2"><h3 className="text-[12px] font-black leading-4">{title}</h3><span className="font-black">›</span></div><p className="mt-1 text-[10px] leading-[13px] text-slate-600">{body}</p></div></Link>})}</div><div className="mt-3 flex items-center justify-between gap-4 rounded-md bg-[#d7f0ff] px-6 py-3"><div><h3 className="text-[15px] font-black">{copy.assessmentLabel}</h3><p className="text-[11px] text-slate-600">{copy.assessmentBody}</p></div><Link href="/assessment" className="shrink-0 rounded-md bg-[#0797e8] px-8 py-3 text-[12px] font-bold text-white shadow">{copy.assessmentCta}</Link></div></div></section>
 </main>;
}