...
export default function HomePageClient(){
 ...
 return <main className="bg-white text-[#061a36]">
  ...
  <section className="mx-auto max-w-[1440px] px-5 py-5 sm:px-8 lg:px-16"><div className="flex items-end justify-between gap-4"><div><h2 className="text-[25px] font-black tracking-[-.02em]">{copy.colorTitle}</h2><p className="text-[13px] text-slate-600">{copy.colorLabel}</p></div><Link href="/curriculum" className="shrink-0 text-[13px] font-bold text-[#0797e8]">{copy.explorePathway} →</Link></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">{copy.tracks.map((track,i)=><Link href={track.href} key={track.code} className="group relative min-h-[154px] overflow-hidden rounded-md bg-[#071426] shadow-md"><img src={trackPhotos[i]} alt="" className="absolute inset-0 h-full w-full object-cover brightness-110 saturate-125 transition group-hover:scale-105"/><div className={`absolute inset-x-0 bottom-0 min-h-[78px] bg-gradient-to-t from-black/95 via-black/75 to-transparent p-3 pt-7`}><h3 className="text-[14px] font-black leading-4 text-white">{track.title}</h3><p className="mt-1 line-clamp-2 text-[10px] leading-[13px] text-white/90">{track.description}</p></div></Link>)}</div></section>
