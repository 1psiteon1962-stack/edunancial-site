...
const TRACK_COLORS: Record<string, { badge: string; heading: string; button: string }> = {
  ...
  BLACK: {
    badge: "text-slate-200 border-slate-400/40 bg-slate-400/10",
          ...
          <div className="space-y-3">
            {lessons.map((lesson, idx) => {
              ...
              <Link
                ...
                className="flex items-start gap-5 rounded-2xl bg-slate-900 border border-slate-800 p-5 hover:border-slate-600 hover:bg-slate-800/70 transition group"
              >
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-sm font-black text-slate-400 group-hover:border-yellow-400 group-hover:text-yellow-400 transition">
                  {idx + 1}
                </div>
                ...
                <span className="flex-shrink-0 text-slate-600 group-hover:text-yellow-400 transition text-lg">
                  →
                </span>
              </Link>
              );
