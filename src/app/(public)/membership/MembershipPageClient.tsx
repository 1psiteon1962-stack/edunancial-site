...
const localPriceNote = (language: string, countryCode: string) => {
...
export default function MembershipPageClient() {
  ...
  return (
    <main className="min-h-screen bg-[#08101f] text-white">
        ...
        <div className="mx-auto max-w-7xl px-6 py-16">
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            {publicMembershipPlans.map((plan) => {
              ...
              const priceLabel = localizedPrice ? `${formatLocalizedPrice(localizedPrice, effectiveLanguage)} ${localizedPrice.currency}` : `${plan.currency} ${plan.monthlyPrice.toFixed(2)}`;
              return (
                <div key={plan.id} className="rounded-2xl border border-white/10 bg-white p-8 text-slate-950 shadow-xl">
                  ...
                  <div className="mt-6"><span className="text-5xl font-black">{priceLabel}</span><span className="ml-2 text-slate-500">{copy.billingLabel}</span></div>
                  {localizedPrice && localizedPrice.currency !== "USD" && <p className="mt-2 text-xs text-slate-500">{localPriceNote(effectiveLanguage, countryCode)}</p>}
                  ...
                  <Link href={plan.showContactOnly ? "/contact" : `/membership/checkout?plan=${plan.id}`} className="mt-10 inline-flex w-full justify-center rounded-xl bg-blue-700 px-6 py-4 font-bold text-white hover:bg-blue-800">{copy.ctaLabel}</Link>
                </div>
              );
            })}
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20"><div className="grid gap-8 md:grid-cols-4">{[1, 2, 3, 4].map((index) => <div key={index} className="rounded-2xl border border-white/10 bg-white/5 p-8"><h3 className="text-2xl font-black">{t(`membership.block${index}.title`)}</h3><p className="mt-4 leading-7 text-slate-300">{t(`membership.block${index}.body`)}</p></div>)}</div></section>
    </main>
  );
}
