...
export default async function TranslationReadinessPage() {
  ...
  const reports = await Promise.all(
    LEVELS.map((level) => getCurriculumTranslationReadiness(level, EXPECTED_LESSONS_PER_LEVEL)),
  );
  const totalTrackLevels = reports.reduce((sum, report) => sum + report.tracks.length, 0);
  const completeTrackLevels = reports.reduce(
    (sum, report) => sum + report.tracks.filter((track) => track.allLanguagesComplete).length,
    0,
  );
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-slate-400">
              Tracks every academy, level, and supported curriculum locale. A translation counts only when the lesson body is actually localized; translated menus, titles, summaries, or English fallback do not count.
            </p>
          </div>
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-4">
          <article className="rounded-2xl border border-white/10 bg-[#101a2f] p-6">
            <p className="text-xs uppercase tracking-wider text-slate-400">Track-Levels Complete</p>
            <p className="mt-3 text-3xl font-black">{completeTrackLevels} / {totalTrackLevels}</p>
          </article>
          <article className="rounded-2xl border border-white/10 bg-[#101a2f] p-6">
            <p className="text-xs uppercase tracking-wider text-slate-400">Levels Tracked</p>
            <p className="mt-3 text-3xl font-black">{LEVELS.length}</p>
          </article>
          <article className="rounded-2xl border border-white/10 bg-[#101a2f] p-6">
            <p className="text-xs uppercase tracking-wider text-slate-400">Expected Lessons / Level</p>
            <p className="mt-3 text-3xl font-black">{EXPECTED_LESSONS_PER_LEVEL}</p>
          </article>
          <article className="rounded-2xl border border-white/10 bg-[#101a2f] p-6">
            <p className="text-xs uppercase tracking-wider text-slate-400">Target Locales</p>
            <p className="mt-3 text-3xl font-black">{targetLocales.length}</p>
          </article>
        </section>
            ...
            return (
              <section key={report.level} className="rounded-2xl border border-white/10 bg-slate-950/20 p-5">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.3em] text-blue-300">Level {report.level}</p>
                    <h2 className="mt-1 text-2xl font-black">{completeTracks} / {report.tracks.length} academies complete</h2>
                  </div>
                  <p className="text-sm text-slate-400">Expected: {report.expectedLessonsPerTrack} canonical lessons per academy</p>
                </div>
                    ...
                    return (
                      <article key={`${report.level}-${track.track}`} className="rounded-2xl border border-white/10 bg-[#101a2f] p-6">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            <h3 className="text-xl font-black">{track.track} · Level {report.level}</h3>
                          ...
                          <details className="mt-5 rounded-xl bg-slate-950/30 p-4">
                            ...
                            <div className="mt-4 space-y-3 text-xs text-slate-400">
                              {incompleteLocales.map((locale) => (
                                <p key={locale.locale}><span className="font-bold text-slate-200">{locale.locale}:</span> {locale.missingLessonIds.join(", ") || `Canonical Level ${report.level} lesson count is below ${report.expectedLessonsPerTrack}.`}</p>
                              ))}
                            </div>
                          </details>
                        ) : null}
