...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await db.from("admin_upload_operations")
    ...
    .limit(100);
  if (error) return Response.json({ success: false, error: error.message }, { status: 503 });
    ...
    for (const upload of uploads) {
      try {
        ...
        await recordUploadOperation({
          ...
          metadata: { recoveryBatchId, recoveredWithoutReupload: true, trustedCanonicalPublication: publication, runner: "github-actions" },
        });
  ...
  return Response.json({ success: failures.length === 0, processed: results.length, failures: failures.length, results }, { status: failures.length ? 207 : 200 });
}
