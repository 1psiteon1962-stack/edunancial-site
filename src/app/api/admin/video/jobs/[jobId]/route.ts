...
export async function GET(request: NextRequest, context: { params: Promise<{ jobId: string }> }) {
  ...
  const { data, error } = await supabase.from("video_jobs").select("id,project_id,status,attempt_count,last_error,started_at,completed_at,output_asset_id,updated_at").eq("id", jobId).single();
