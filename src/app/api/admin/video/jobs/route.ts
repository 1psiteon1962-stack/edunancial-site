...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: job, error: jobError } = await supabase
      .from("video_jobs")
      .upsert({ project_id: projectId, source_asset_id: source.id, stage: "RENDER_MASTER", status: "queued", edit_recipe: editRecipe, last_error: null, completed_at: null }, { onConflict: "project_id,stage" })
      .select("id,status")
      .single();
