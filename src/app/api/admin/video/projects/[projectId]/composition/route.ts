...
export async function PUT(request: NextRequest, context: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    const backgroundMusic = parseAudio(body.backgroundMusic, "und", 0.18);
    ...
    await supabase.from("video_audio_tracks").delete().eq("project_id", projectId).in("track_type", ["ORIGINAL_NARRATION", "BACKGROUND_MUSIC"]);
    const audioTracks = [];
    if (narration) audioTracks.push({ project_id: projectId, asset_id: narration.assetId, track_type: "ORIGINAL_NARRATION", locale: narration.locale || "en-US", transcript: narration.transcript || null, volume: narration.volume, muted: false });
    if (backgroundMusic) audioTracks.push({ project_id: projectId, asset_id: backgroundMusic.assetId, track_type: "BACKGROUND_MUSIC", locale: backgroundMusic.locale || "und", transcript: null, volume: backgroundMusic.volume, muted: false });
    if (audioTracks.length) {
      const { error: audioError } = await supabase.from("video_audio_tracks").insert(audioTracks);
