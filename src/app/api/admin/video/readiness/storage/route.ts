...
export async function POST(request: NextRequest) {
  ...
  for (const bucket of REQUIRED_VIDEO_BUCKETS) {
    ...
    if (error) {
      return Response.json(
        {
          success: false,
          error: `Could not create private storage bucket ${bucket}: ${error.message}`,
          created,
        },
        { status: 500 },
      );
  ...
  return Response.json({
    success: true,
    created,
    message: created.length ? `Created private bucket${created.length === 1 ? "" : "s"}: ${created.join(", ")}` : "Required private video storage buckets already exist.",
  });
}
