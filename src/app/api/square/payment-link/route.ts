...
import { resolveRequestCountry } from "@/lib/location/request-country";
...
export async function POST(request:Request){
 ...
 try{
  ...
  const countryResolution=resolveRequestCountry(request.headers,body.countryCode);
  if(!countryResolution)return attachRequestHeaders(NextResponse.json({success:false,error:"Your country could not be verified. Select your country before checkout or retry without a network location blocker.",requestId},{status:422}),requestId);
  if(countryResolution.mismatch)return attachRequestHeaders(NextResponse.json({success:false,error:`Your selected country (${countryResolution.requestedCountryCode}) does not match the detected checkout country (${countryResolution.detectedCountryCode}). Confirm your location before payment.`,selectedCountry:countryResolution.requestedCountryCode,detectedCountry:countryResolution.detectedCountryCode,countrySource:countryResolution.source,requestId},{status:409}),requestId);
  ...
  await persistCheckoutInitiation({item,customerEmail,amountRequested:finalPrice+(taxMinor/100),currency:item.currency,discountCode,discountAmount:item.price-finalPrice,squarePaymentLinkId:squareData.payment_link?.id,squareOrderId:squareData.payment_link?.order_id,idempotencyKey,metadata:{requestId,itemType:item.type,countryCode:countryControl.countryCode,countrySource:countryResolution.source,countryLaunchState:countryControl.launchState,taxStatus,taxMinor,jurisdictionKey:jurisdictionKey||null}});
  if(discountApplied&&discountCode)recordDiscountRedemption(discountCode);
  const response=NextResponse.json({success:true,checkoutUrl,countryCode:countryControl.countryCode,countrySource:countryResolution.source,jurisdictionKey:jurisdictionKey||undefined,taxMinor,taxCurrency:currency,taxStatus,itemId:item.id,itemType:item.type,planId:item.membershipPlanId,contentId:item.contentId,originalPrice:item.price,finalPrice,discountApplied,squarePaymentLinkId:squareData.payment_link?.id,squareOrderId:squareData.payment_link?.order_id,requestId});recordRequestMetric({method:request.method,route,status:200,durationMs:Date.now()-start});return attachRequestHeaders(response,requestId);
 }catch(error){logStructuredError(error,{...getRequestContext(request,requestId),route});const response=NextResponse.json({success:false,error:"Square checkout could not be activated. Verified payment, persistence, tax, and country prerequisites must succeed before payment is accepted.",requestId},{status:503});recordRequestMetric({method:request.method,route,status:503,durationMs:Date.now()-start});return attachRequestHeaders(response,requestId);}
}
