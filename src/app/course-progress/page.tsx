  ...
  return (
    <BilingualContent
                ...
                <div key={course.id} className="rounded-2xl bg-slate-900 border border-slate-800 p-6">
                  ...
                  <div className="flex items-center justify-between text-sm text-slate-400">
                    ...
                    {course.pct === 100 ? (
                      ...
                      <Link href={`/courses/${course.id}/lessons/${course.lessons[course.done] ?? course.lessons[0]}`} className="text-yellow-400 font-bold hover:text-yellow-300">
                        Continue →
                      </Link>
                    )}
                  </div>
      ...
      es={
        <main className="min-h-screen bg-[#08101f] text-white">
          <section className="mx-auto max-w-7xl px-6 py-20">
            <p className="uppercase tracking-[0.45em] font-bold text-yellow-400 text-sm">SEGUIMIENTO DE PROGRESO</p>
            <h1 className="mt-4 text-5xl font-black">Progreso del curso</h1>

            <div className="mt-10 rounded-2xl bg-slate-900 border border-slate-800 p-8">
              <div className="flex items-end justify-between mb-4">
                <div>
                  <p className="text-slate-400 text-sm">Progreso general de aprendizaje</p>
                  <p className="text-4xl font-black mt-1 text-blue-400">{overallPct}%</p>
                </div>
                <p className="text-slate-400 text-sm">{totalDone} / {totalLessons} lecciones</p>
              </div>
              <div className="h-4 w-full rounded-full bg-slate-800">
                <div className="h-4 rounded-full bg-gradient-to-r from-blue-600 to-blue-400 transition-all" style={{ width: `${overallPct}%` }} />
              </div>
            ...
            <div className="mt-10 space-y-6">
              <h2 className="text-2xl font-black">Desglose por curso</h2>
              {enrolled.map((course) => (
                <div key={course.id} className="rounded-2xl bg-slate-900 border border-slate-800 p-6">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                  ...
                  <div className="flex items-center justify-between text-sm text-slate-400">
                    <span>{course.done} de {course.lessons.length} lecciones completas</span>
                    {course.pct === 100 ? (
                      <Link href="/my-certificates" className="text-green-400 font-bold hover:text-green-300">
