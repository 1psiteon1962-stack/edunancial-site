        ...
        <main className="min-h-screen bg-[#08101f] text-white">
          <section className="mx-auto max-w-7xl px-6 py-24">
            ...
            <h1 className="mt-4 text-5xl font-black md:text-7xl">
              Construya competencia financiera
            </h1>
            <p className="mt-6 max-w-3xl text-xl text-slate-300 leading-relaxed">
              La alfabetización financiera aporta la base.
              La competencia financiera se construye con acción disciplinada.
            </p>
            ...
            <div className="mt-16 grid gap-6 md:grid-cols-3">
              {[
                ...
                <Link key={track.color} href={track.href} className={`${track.bg} rounded-2xl p-10 hover:opacity-90 transition`}>
                  <h2 className="text-5xl font-black">{track.color}</h2>
                  <h3 className="mt-6 text-2xl font-bold">{track.title}</h3>
                  <p className="mt-4 text-lg opacity-80">{track.description}</p>
                  <p className="mt-8 font-bold text-sm">Explorar ruta →</p>
                </Link>
              ))}
            </div>

            <div className="mt-16 rounded-2xl border border-blue-400/30 bg-blue-500/10 p-8 text-slate-200">
              <h2 className="text-2xl font-black text-white">Detalles completos en español en desarrollo</h2>
              <p className="mt-4 max-w-3xl leading-8">
                El catálogo completo de cursos y las páginas de detalle en español para
                Norteamérica continúan en preparación. Mientras tanto, cada ruta RED, WHITE y BLUE
                ya muestra el panorama general, los recursos planeados y un siguiente paso claro.
              </p>
            </div>

            <div className="mt-12 flex flex-col sm:flex-row gap-4">
              <Link href="/membership" className="rounded-xl bg-yellow-500 px-8 py-4 font-black text-black text-center hover:bg-yellow-400 transition">
                Ver membresías
              </Link>
              <Link href="/course-progress" className="rounded-xl border border-slate-600 px-8 py-4 font-bold text-slate-300 text-center hover:bg-slate-800 transition">
                Ver progreso
              </Link>
            </div>
          </section>
