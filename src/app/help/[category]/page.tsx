...
export async function generateMetadata({
  ...
  const category = helpCategories.find((entry) => entry.slug === categorySlug);
...
export default async function HelpCategoryPage({ params }: HelpCategoryPageProps) {
  ...
  const categoryArticles = helpArticles.filter((article) => article.category === category.id);
  const relatedCategories = helpCategories.filter((entry) => entry.id !== category.id);
