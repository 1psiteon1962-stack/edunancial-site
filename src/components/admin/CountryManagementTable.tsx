...
import { getRolloutPhaseForCountry } from "@/lib/countries/rollout-phases";
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">
          Configuration status is separate from rollout phase, launch readiness, and commercial readiness. Markets remain blocked until all required readiness dimensions are complete.
        </p>
      </div>
        ...
        <table className="w-full min-w-[1450px]">
          <thead>
            <tr className="border-b text-sm text-slate-600">
              ...
              <th className="py-3 text-left">Rollout Phase</th>
              <th className="py-3 text-left">Priority Locales</th>
          ...
          <tbody>
            {countries.map((country) => {
              const phase = getRolloutPhaseForCountry(country.isoCode);
              ...
              return (
                <tr key={country.isoCode} className="border-b align-top">
                  ...
                  <td className="py-4 pr-5 text-sm text-slate-700">
                    {phase ? `${phase.order}. ${phase.label}` : "Unscheduled"}
                  </td>
                  <td className="py-4 pr-5 text-sm text-slate-600">
                    {phase?.priorityLocales.join(", ") ?? "—"}
                  </td>
