...
const AILearningContextContainer = createContext<AILearningContextValue>({
  ...
  askCoach: async () => ({ enabled: false, message: "AI Learning Network unavailable.", suggestions: [], disclaimers: [], milestone: null, contextSummary: "" }),
});

function resolveMembershipStatus(value: string | undefined): MembershipStatus {
  if (!value) return "public";
  if (value === "free" || value === "basic" || value === "premium" || value === "enterprise" || value === "beta") return value;
  return "public";
}

function loadAdminConfig(): AILearningAdminConfig {
  if (typeof window === "undefined") return DEFAULT_AI_LEARNING_CONFIG;
  try {
    const raw = localStorage.getItem(AI_LEARNING_ADMIN_STORAGE_KEY);
    return raw ? mergeAILearningConfig(JSON.parse(raw) as Partial<AILearningAdminConfig>) : DEFAULT_AI_LEARNING_CONFIG;
  } catch {
...
export function AILearningProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => {
      if (event.key === AI_LEARNING_ADMIN_STORAGE_KEY) setConfig(loadAdminConfig());
    };
  ...
  useEffect(() => {
    ...
    const merged = mergeContextAcrossNavigation(safeLoadContextFromSessionStorage(), current);
  ...
  const value = useMemo<AILearningContextValue>(() => ({
    ...
    askCoach: async (message: string) => {
      if (!context) return { enabled: false, message: "AI context is still loading.", suggestions: [], disclaimers: [], milestone: null, contextSummary: "" };
      const response = await fetch("/api/ai-learning/respond", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // Policy/configuration is intentionally not transmitted. The server owns
        // availability, safety and jurisdiction rules; the browser provides context.
        body: JSON.stringify({ message, context }),
      });
      if (!response.ok) return { enabled: false, message: "AI Learning Network is temporarily unavailable.", suggestions: ["Try again shortly"], disclaimers: [], milestone: null, contextSummary: context.track ?? "" };
      return (await response.json()) as AILearningResponse;
    },
  }), [context, config]);

  return <AILearningContextContainer.Provider value={value}>{children}</AILearningContextContainer.Provider>;
}

export function useAILearning() { return useContext(AILearningContextContainer); }
