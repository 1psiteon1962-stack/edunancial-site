...
interface CheckoutFormProps { plan: MembershipPlan; secureCheckoutEnabled: boolean; }
...
export default function CheckoutForm({ plan, secureCheckoutEnabled }: CheckoutFormProps) {
  ...
  const displayPrice = localizedPrice ? `${formatLocalizedPrice(localizedPrice, effectiveLanguage)} ${localizedPrice.currency}` : `${plan.currency} ${plan.monthlyPrice.toFixed(2)}`;
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-10 text-slate-900 shadow-sm">
    ...
    <div className="mt-8 space-y-3 rounded-xl bg-slate-50 p-6">
      ...
      <div className="flex justify-between"><span className="text-slate-600">{t("checkout.monthlyPrice")}</span><strong>{displayPrice}</strong></div><hr className="border-slate-200" />
      <div className="flex justify-between text-lg font-bold"><span>{t("checkout.totalToday")}</span><span>{displayPrice}</span></div>
    </div>
    {localizedPrice && localizedPrice.currency !== "USD" && <p className="mt-3 text-xs text-slate-500">Localized {localizedPrice.currency} price for {countryCode}. Applicable taxes are calculated at checkout. Payment remains unavailable until an approved {localizedPrice.currency} settlement route is configured.</p>}
...
export function CheckoutPage({ planId, secureCheckoutEnabled }: { planId?: string; secureCheckoutEnabled: boolean }) {
  ...
  return <main className="mx-auto max-w-3xl px-6 py-16 text-white"><nav className="mb-8 flex gap-2 text-sm text-slate-400"><Link href="/membership" className="hover:text-white">{t("checkout.membership")}</Link><span>/</span><span className="text-white">{t("checkout.checkout")}</span></nav><h1 className="text-4xl font-black">{t("checkout.checkout")}</h1><p className="mt-2 text-slate-400">{t("checkout.youSelected")} <strong className="text-white">{selectedPlanCopy.name}</strong>{t("checkout.planSuffix")}</p><div className="mt-10"><CheckoutForm plan={selectedPlan} secureCheckoutEnabled={secureCheckoutEnabled} /></div><div className="mt-8 grid gap-4 text-sm text-slate-400 sm:grid-cols-3">{[{ icon: "🔒", text: t("checkout.sslEncryption") },{ icon: "📋", text: t("checkout.cancelLabel") },{ icon: "✅", text: t("checkout.accessGranted") }].map(({ icon, text }) => <div key={text} className="flex items-center gap-2"><span>{icon}</span><span>{text}</span></div>)}</div></main>;
}
