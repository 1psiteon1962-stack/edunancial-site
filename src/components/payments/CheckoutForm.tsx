...
export default function CheckoutForm({ plan, secureCheckoutEnabled }: CheckoutFormProps) {
  const { user } = useAuth();
  ...
  const catalogItemId = `membership-${plan.id}-monthly`;
  const yesLabel = "✓";
  const noLabel = "✗";
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-10 text-slate-900 shadow-sm">
      ...
      <div className="mt-8 space-y-3 rounded-xl bg-slate-50 p-6">
        ...
        <div className="flex justify-between"><span className="text-slate-600">{t("checkout.billingPeriod")}</span><strong>{planCopy.billingLabel}</strong></div>
        <div className="flex justify-between"><span className="text-slate-600">{t("checkout.monthlyPrice")}</span><strong>{displayPrice}</strong></div>
      ...
      {localizedPrice && localizedPrice.currency !== "USD" && (
        <p className="mt-3 text-xs text-slate-500">
          Localized {localizedPrice.currency} price for {countryCode}. Applicable taxes are calculated at checkout. Payment remains unavailable until an approved {localizedPrice.currency} settlement route is configured.
        </p>
      )}
      ...
      {!user && (
        <div className="mt-6 rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3 text-sm text-yellow-800">
          ...
          {t("checkout.or")} <Link href="/register" className="font-semibold underline">{t("checkout.createAccount")}</Link>{" "}{t("checkout.beforePurchasing")}
        </div>
      )}
      ...
      {plan.showContactOnly ? (
        <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 px-4 py-4 text-sm text-blue-900">{planCopy.legalNote ?? t("checkout.requiresApproval")} {t("checkout.contactSupport")}</div>
        ...
        <div className="mt-6">
          <PaymentLinkCheckoutButton
            ...
            label={`Continue to secure checkout — ${displayPrice}`}
            className="flex w-full items-center justify-center rounded-xl bg-blue-700 px-6 py-4 font-semibold text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-50"
          />
        </div>
      ) : !user ? (
        <div className="mt-6 rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-4 text-sm text-yellow-800">{t("checkout.signInBeforeCheckout")}</div>
      ) : (
        <div className="mt-6 rounded-lg border border-amber-300 bg-amber-50 px-4 py-4 text-sm text-amber-900">{t("checkout.checkoutUnavailable")}</div>
      )}

      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <Link href="/membership" className="inline-flex flex-1 items-center justify-center rounded-xl border border-slate-300 px-4 py-3 text-center font-bold text-slate-900 hover:border-slate-400">{t("checkout.backToMembership")}</Link>
        <Link href="/contact" className="inline-flex flex-1 items-center justify-center rounded-xl bg-blue-700 px-4 py-3 text-center font-bold text-white hover:bg-blue-800">{plan.showContactOnly ? t("checkout.requestOrgApproval") : t("checkout.contactSupportCta")}</Link>
      </div>
...
export function CheckoutPage({ planId, secureCheckoutEnabled }: { planId?: string; secureCheckoutEnabled: boolean }) {
  const canonicalPlanId = resolveMembershipPlanId(planId);
  const selectedPlan = membershipPlans.find((plan) => plan.id === canonicalPlanId) ?? membershipPlans[0];
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16 text-white">
      <nav className="mb-8 flex gap-2 text-sm text-slate-400"><Link href="/membership" className="hover:text-white">{t("checkout.membership")}</Link><span>/</span><span className="text-white">{t("checkout.checkout")}</span></nav>
      <h1 className="text-4xl font-black">{t("checkout.checkout")}</h1>
      <p className="mt-2 text-slate-400">{t("checkout.youSelected")} <strong className="text-white">{selectedPlanCopy.name}</strong>{t("checkout.planSuffix")}</p>
      <div className="mt-10"><CheckoutForm plan={selectedPlan} secureCheckoutEnabled={secureCheckoutEnabled} /></div>
      <div className="mt-8 grid gap-4 text-sm text-slate-400 sm:grid-cols-3">
        {[{ icon: "🔒", text: t("checkout.sslEncryption") },{ icon: "📋", text: t("checkout.cancelLabel") },{ icon: "✅", text: t("checkout.accessGranted") }].map(({ icon, text }) => <div key={text} className="flex items-center gap-2"><span>{icon}</span><span>{text}</span></div>)}
      </div>
    </main>
  );
}
