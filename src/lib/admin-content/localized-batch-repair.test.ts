...
function lessonFile(overrides: Partial<ExtractedFile> = {}): ExtractedFile {
  ...
  return {
    ...
    batchId: "batch-1",
    uploadId: "upload-1",
    originalFilename: "BLACK-L1-001.md",
    normalizedFilename: "black-l1-001.md",
    archivePath: "BLACK-L1-001.md",
    sourceArchiveFilename: "BLACK_L1_pt-BR_50_CURRICULUM_COMPLETO.zip",
    extension: ".md",
    ...
    previewText: body,
    rawText: body,
    encodedContent: Buffer.from(body).toString("base64"),
    classification: {
      category: "lessons",
      subcategory: "level-1",
      language: "en-US",
      academyLevel: "level-1",
      destination: "content/courses/black/level-1/en-US/black-l1-001.md",
      confidence: 1,
      reasons: [],
      pillar: "black",
    },
    metadata: {
      ...
      description: "",
      source: "upload",
      intendedDestination: "content/courses/black/level-1/en-US/black-l1-001.md",
      contentType: "lessons",
      pillar: "black",
      academyLevel: "level-1",
...
describe("localized batch publication", () => {
  ...
  test("publishes regional English such as en-GB as localized content", () => {
    const file = lessonFile({
      sourceArchiveFilename: "BLACK_L1_en-GB_50_CURRICULUM_COMPLETE.zip",
    });
  ...
  test("does not turn canonical en-US packages into translations", () => {
    const file = lessonFile({
      sourceArchiveFilename: "BLACK_L1_en-US_50_CURRICULUM_COMPLETE.zip",
    });
  ...
  test("supports the production locale families used by curriculum ZIPs", () => {
    const cases: Array<[string, string]> = [
      ["BLACK_L1_fr-CA_50_CURRICULUM_COMPLET.zip", "fr-CA"],
      ["BLACK_L1_fr-FR_50_CURRICULUM_COMPLET.zip", "fr-FR"],
      ["BLACK_L1_es-ES_50_CURRICULO_COMPLETO.zip", "es-ES"],
      ["BLACK-L1-es-Caribbean-COMPLETE.zip", "es-Caribbean"],
      ["BLACK_L1_pt-PT_50_CURRICULO_COMPLETO.zip", "pt-PT"],
      ["BLACK_L1_de-DE_50_CURRICULUM_VOLLSTAENDIG.zip", "de"],
      ["BLACK_L1_nl-NL_50_CURRICULUM_VOLLEDIG.zip", "nl"],
      ["BLACK_L1_it-IT_50_CURRICULUM_COMPLETO.zip", "it"],
    ];
    for (const [archive, expected] of cases) {
      assert.equal(resolveLocalizedFileLocale(lessonFile({ sourceArchiveFilename: archive })), expected, archive);
    }
  });
});
