...
describe("admin-content storage env resolution", () => {
  ...
  test("accepts the explicit bucket alias EDUNANCIAL_UPLOAD_STORAGE_BUCKET", () => {
    delete process.env.EDUNANCIAL_UPLOAD_STORAGE_KEY;
  ...
  test("prefers EDUNANCIAL_UPLOAD_STORAGE_KEY over EDUNANCIAL_UPLOAD_STORAGE_BUCKET", () => {
    env.EDUNANCIAL_UPLOAD_STORAGE_KEY = "primary-bucket";
    env.EDUNANCIAL_UPLOAD_STORAGE_BUCKET = "alias-bucket";

    const config = resolveAdminContentStorageEnv();
    assert.equal(config.bucket, "primary-bucket");
  });
