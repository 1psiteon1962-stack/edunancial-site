...
export async function createIndependentUploadBatchFromStoredFiles(
  ...
  // Curriculum ZIPs are trusted, explicitly classified packages. Do not
  // synchronously download up to 50 historical batch JSON documents just to
  // populate review-time conflict hints. That remote fan-out was on the
  // critical HTTP finalization path and could exhaust the gateway deadline for
  // 50-lesson packages. Publication has its own canonical curriculum identity
  // and deduplication safeguards.
  if (uploadConfig.destination !== "courses") {
