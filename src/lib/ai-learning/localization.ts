...
- Learning jurisdiction selected by the learner: ${jurisdictionName} (${scope.learningJurisdiction})
- Canonical/source jurisdiction: ${sourceJurisdictionName} (${scope.sourceJurisdiction})
...
3. Preserve universal concepts when they remain accurate. Change only what materially differs in ${jurisdictionName}.
4. Never infer jurisdiction from language. The selected learning jurisdiction above controls substantive law, tax, regulatory, institutional, and market context. The display language controls presentation only.
5. Do not silently substitute ${sourceJurisdictionName} rules for ${jurisdictionName}. If a jurisdiction-specific fact cannot be verified, say so clearly and keep the explanation at the universal-concept level.
...
export function isQuestionWithinLessonScope(
  message: string,
  lessonText: string,
  lessonTitle?: string | null,
): boolean {
  ...
  const scopeTerms = `${lessonTitle ?? ""} ${lessonText}`
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, " ")
    .split(/\s+/)
  ...
  // Short coaching prompts such as "explain this", "give me an example", or
  // "how does this work here?" are valid because the active lesson supplies scope.
  return /\b(this|lesson|example|explain|here|country|jurisdiction|compare|difference|formula|mean)\b/i.test(
    normalizedMessage,
  );
}
