...
import { normalizeJurisdiction, type AILearningContext, type MembershipStatus } from "./context";
...
export interface AILearningPipelineResult extends AILearningResponse {
  pipeline: { version: "1.0"; stages: AILearningPipelineStage[] };
}
...
function normalizeMembership(value: unknown): MembershipStatus {
  return MEMBERSHIPS.has(value as MembershipStatus) ? (value as MembershipStatus) : "public";
}
...
function normalizeCompletedLessons(value: unknown): string[] {
  ...
  return Array.from(new Set(value.slice(0, MAX_COMPLETED_LESSONS).map((entry) => normalizeLessonId(entry)).filter((entry): entry is string => Boolean(entry))));
}
...
export function normalizeAILearningContext(context: AILearningContext): AILearningContext {
  ...
  return {
    ...
    track: normalizeTrack(context.track),
    level: normalizeLevel(context.level),
    lessonId: normalizeLessonId(context.lessonId),
    topic: cleanText(context.topic, 160) || null,
    ...
    certificationPath: cleanText(context.certificationPath, 80) || null,
...
export async function runAILearningPipeline(input: AILearningPipelineInput): Promise<AILearningPipelineResult> {
  ...
  if (!message) {
    return { enabled: false, message: "Please enter a learning question.", suggestions: [], disclaimers: [], milestone: null, contextSummary: "", pipeline: { version: "1.0", stages } };
  }
  ...
  const response = await generateAILearningResponse({ message, context, config });
