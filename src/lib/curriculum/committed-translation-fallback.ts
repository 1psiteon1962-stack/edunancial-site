...
type RuntimeMarkdownRecord = { path: string; content: string };
let runtimeMarkdown: readonly RuntimeMarkdownRecord[] | null = null;

function getCurriculumRuntimeMarkdown(): readonly RuntimeMarkdownRecord[] {
  ...
  try {
    runtimeMarkdown = existsSync(manifestPath)
      ? (JSON.parse(readFileSync(manifestPath, "utf8")) as RuntimeMarkdownRecord[])
      : [];
  } catch {
    runtimeMarkdown = [];
  }
...
function bundledTranslationLookup(lessonId: string, languageOrLocale: string): PublishedLessonTranslation | undefined {
  ...
  for (const candidateLocale of getCurriculumLocaleFallbackChain(requested)) {
    ...
    for (const record of getCurriculumRuntimeMarkdown()) {
