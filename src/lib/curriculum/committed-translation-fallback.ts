...
// Lesson text generated by scripts/curriculum/generate-runtime-manifest.mjs at prebuild.
// Loaded lazily from JSON instead of being imported as a TypeScript module: importing
// ~13 MB of string literals pushed `next build` past the V8 heap limit (exit 134).
type RuntimeContentRecord = { path: string; content: string };
let runtimeContent: readonly RuntimeContentRecord[] | null = null;

function getCurriculumRuntimeContent(): readonly RuntimeContentRecord[] {
  if (runtimeContent) return runtimeContent;
  const manifestPath = join(REPO_ROOT, "content", "generated", "curriculum-runtime-manifest.json");
  try {
    runtimeContent = existsSync(manifestPath)
      ? (JSON.parse(readFileSync(manifestPath, "utf8")) as RuntimeContentRecord[])
      : [];
  } catch {
    runtimeContent = [];
  }
...
function parseLegacyTranslationJson(
  raw: string,
  lessonId: string,
  locale: string,
  ...
  try {
    ...
    for (const key of candidates) {
      ...
      if (translation?.title?.trim() && translation.body?.trim()) return translation;
    }
  } catch {
    return null;
  }
...
function parseRuntimeRecord(
  record: RuntimeContentRecord,
  lessonId: string,
  locale: string,
): PublishedLessonTranslation | null {
  return record.path.toLowerCase().endsWith(".json")
    ? parseLegacyTranslationJson(record.content, lessonId, locale)
    : parseMarkdown(record.content, lessonId);
}
...
function bundledTranslationLookup(lessonId: string, languageOrLocale: string): PublishedLessonTranslation | undefined {
  ...
  for (const candidateLocale of getCurriculumLocaleFallbackChain(requested)) {
    ...
    for (const record of getCurriculumRuntimeContent()) {
      ...
      const translation = parseRuntimeRecord(record, lessonId.trim().toUpperCase(), normalized);
