...
const lesson: PublishedLessonRecord = {
  ...
  title: "English title", summary: "English summary", author: "Edunancial Faculty", date: "2026-08-30", version: "1.0", status: "active",
  importedAt: "2026-08-30T00:00:00.000Z", metadata: {}, path: "content/curriculum/BLACK/L1/BLACK-L1-001.md", body: "English body", frontMatter: {},
};

describe("runtime curriculum localization", () => {
  it("restores a complete regional translation from historical uploads", () => {
    const value = index();
    addHistoricalTranslation(value, lesson.id, "es-Caribbean", { title: "Titulo en espanol", summary: "Resumen en espanol", body: "Cuerpo completo en espanol" });
    const translated = applyHistoricalTranslation(lesson, "es-Caribbean", value);
    assert.deepEqual({ title: translated.title, summary: translated.summary, body: translated.body }, { title: "Titulo en espanol", summary: "Resumen en espanol", body: "Cuerpo completo en espanol" });
  });

  it("uses a regional translation when a base language is requested", () => {
    const value = index(); addHistoricalTranslation(value, lesson.id, "pt-BR", { body: "Corpo em portugues" });
  ...
  it("keeps newer fields while older duplicate batches only fill gaps", () => {
    ...
    assert.deepEqual(resolveHistoricalTranslation(value, lesson.id, "fr-CA"), { title: "Titre recent", summary: "Resume ancien", body: "Corps recent" });
  });
});
