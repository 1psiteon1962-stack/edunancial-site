...
export function calculateInvestmentGrowth(input: InvestmentGrowthInput): InvestmentGrowthResult {
  ...
  for (let year = 1; year <= years; year += 1) {
    ...
    yearly.push({ year, contributed: totalContributed, growth: balance - totalContributed, endingBalance: balance });
  }

  return { totalContributed, totalGrowth: balance - totalContributed, endingBalance: balance, yearly };
}
