...
export async function buildLessonLocalizationContext(
  ...
  const rules = await repo.findRules({
    jurisdiction: selection.countryCode,
    subdivisionCode: selection.subdivisionCode,
    topics,
    asOf,
  });
  ...
  const usableRules = rules.filter(rule =>
    ...
    (!rule.effectiveFrom || rule.effectiveFrom <= asOf) &&
  ...
  const requiresHumanReview =
    groundedRules.some(rule => rule.risk !== 'green') ||
    rules.some(rule => rule.verificationStatus === 'conflict') ||
    groundedRules.length === 0;
  ...
  return {
    ...
    topics,
    rules: groundedRules,
    sources: groundedRules.length ? sources : [],
    generatedAt: asOf,
    requiresHumanReview,
  };
}
