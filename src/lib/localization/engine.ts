...
// ─── Compatibility aliases for PR#53 modules ─────────────────────────────────
// legalText.ts and currency.ts use RegionCode + REGION_REGISTRY naming.
// These aliases map RegionSlug → RegionCode and provide a compatible REGION_REGISTRY.

/** Alias for RegionSlug — use RegionSlug in new code. */
export type RegionCode = RegionSlug;

/** Registry shape compatible with currency.ts and legalText.ts. */
...
const COUNTRY_TO_CURRENCY: Partial<Record<RegionSlug, string>> = {
  "north-america": "USD",
  "europe-2a": "EUR",
  "europe-2b": "GBP",
  africa: "USD",
...
function buildRegionDefinition(slug: RegionSlug): RegionDefinition {
  const cfg = REGION_CONFIGS.find((r) => r.slug === slug);
  const sample = resolveRegionalization({ region: slug });
  return {
    code: slug,
    name: cfg?.name ?? slug,
    enabled: cfg?.enabled ?? false,
    defaultLocale: sample.language,
    locales: cfg?.supportedLanguages ?? ["en"],
    currency: sample.currency ?? COUNTRY_TO_CURRENCY[slug] ?? "USD",
    currencies: cfg?.supportedCurrencies ?? ["USD"],
    intlLocale: "en",
    rtl: slug === "middle-east",
  };
}

export const REGION_REGISTRY: Record<RegionSlug, RegionDefinition> = (() => {
  const allSlugs: RegionSlug[] = [
    "north-america",
    "europe-2a",
    "europe-2b",
    "africa",
  ...
  ) as Record<RegionSlug, RegionDefinition>;
})();
