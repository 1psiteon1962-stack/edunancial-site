...
test("payment-link route accepts the square payment test item and sends 100 cents USD to Square", async () => {
  let capturedUrl = "";
  ...
  assert.equal(body.itemId, "square-payment-test-001");
  assert.equal(body.itemType, "other");
  assert.equal(
    capturedUrl,
  ...
  assert.equal("membership_plan_id" in order.metadata, false);

  const redirectUrl = new URL(checkoutOptions.redirect_url);
  assert.equal(redirectUrl.origin, "https://edunancial.com");
  assert.equal(redirectUrl.pathname, "/payment/success");
  assert.equal(redirectUrl.searchParams.get("item"), "square-payment-test-001");
  assert.equal(redirectUrl.searchParams.get("type"), "other");
});
