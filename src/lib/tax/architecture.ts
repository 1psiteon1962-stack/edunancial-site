export type TaxCountryCode = "US" | "CA";
...
export type TaxComponentKind = "sales-tax" | "gst" | "hst" | "pst" | "rst" | "qst";
...
export interface NexusThreshold {
  ...
  subdivisionCode?: string;
  currency: string;
  salesThresholdMinor?: number;
  transactionThreshold?: number;
  lookback: "calendar-year" | "previous-calendar-year" | "rolling-12-months" | "other";
  effectiveFrom: string;
  effectiveTo?: string;
}
...
export interface TaxQuoteRequest {
  sellerCountryCode: TaxCountryCode;
  customer: TaxAddress;
...
export interface TaxEngineAdapter {
  readonly countryCode: TaxCountryCode;
  quote(request: TaxQuoteRequest): TaxQuote;
}

export function jurisdictionKey(address: TaxAddress): string {
  return [address.countryCode, address.subdivisionCode, address.postalCode]
    .filter(Boolean)
    .join("-")
    .toUpperCase();
}
...
export function shouldCollectTax(
  ...
  nexus: NexusSnapshot | undefined,
): boolean {
  if (registration?.status === "registered") return true;
  if (registration?.status === "required") return true;
  if (!nexus?.threshold) return false;

  const salesReached =
    nexus.threshold.salesThresholdMinor !== undefined &&
    nexus.taxableSalesMinor >= nexus.threshold.salesThresholdMinor;
  const transactionsReached =
    nexus.threshold.transactionThreshold !== undefined &&
    nexus.transactionCount >= nexus.threshold.transactionThreshold;

  return salesReached || transactionsReached;
}

export function createZeroTaxQuote(request: TaxQuoteRequest, reason: string): TaxQuote {
  return {
    jurisdictionKey: jurisdictionKey(request.customer),
    taxable: false,
    collectionRequired: false,
    subtotal: request.subtotal,
    tax: { amountMinor: 0, currency: request.subtotal.currency },
