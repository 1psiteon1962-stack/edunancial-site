...
function verifySignature(req, path, body) {
  ...
  if (typeof timestamp !== "string" || typeof requestId !== "string" || typeof signature !== "string") {
    throw new Error("Missing worker authentication headers");
  }
...
function run(cmd, args) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => code === 0 ? resolve() : reject(new Error(`${cmd} exited ${code}: ${stderr.slice(-4000)}`)));
  });
}
...
async function executeJob(jobId, requestId) {
  ...
  try {
    ...
    for (const [index, scene] of scenes.entries()) {
      ...
      await run("ffmpeg", ["-y", ...inputArgs, "-vf", vf, "-an", "-c:v", "libx264", "-preset", "veryfast", "-movflags", "+faststart", output]);
      sceneFiles.push({ output, duration, transition: scene.transition_type || "cut", transitionSeconds: Number(scene.transition_seconds || 0.35) });
    }
    ...
    await run("ffmpeg", ["-y", "-f", "concat", "-safe", "0", "-i", listFile, "-c", "copy", visual]);
    ...
    if (!narration && !music) {
      await run("ffmpeg", ["-y", "-i", visual, "-c", "copy", output]);
    } else {
      ...
      if (narrationLabel && musicLabel) {
        filters.push(`${narrationLabel}${musicLabel}amix=inputs=2:duration=longest:dropout_transition=2[aout]`);
        audioMap = "[aout]";
      } else {
        audioMap = narrationLabel || musicLabel;
      }
...
const server = http.createServer(async (req, res) => {
  try {
    if (req.method === "GET" && req.url === "/health") {
      requireConfig();
      return json(res, 200, { ok: true, service: "edunancial-video-worker" });
    }
    ...
    return json(res, 500, { ok: false, error: error instanceof Error ? error.message : "Worker failure" });
  }
});
